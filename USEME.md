...
- No command line arguments are required to run the program, and the program will default to using a GUI
  - "-text" will run the program in text mode
  - "-file" "path/to/file" will run the commands in the inputted script file and then exit.
...
## GUI HOW TO USE:
(1) '**load**' 
- Hover over the file button in the top left of the window, and then click on load in the dropdown menu.
...
(2) '**save**' 
- Hover over the file button in the top left of the window, and then click on save in the dropdown menu.
...
(3) '**adjust-brightness**' 
- Click on the adjust brightness button and then pass in a valid number for the increment. The increment can be positive or negative. An invalid increment will require the user to re-enter a valid increment.
...
(4) '**vertical-flip**' 
- Choose vertical flip from the flip dropdown menu and then click "Choose Flip Command"
